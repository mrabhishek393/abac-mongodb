...
class MongoABAC
  ...
  def abac(msg, user_name, ip)
    ...
    json1 = File.read('user_attributes.json')
    ...
    if not user_attributes
      ...
      nil 
      encrypted = File.read('encrypted_ua.txt')
      key = File.read ('encrypted_key.txt')
      iv = File.read ('encrypted_iv.txt')
    ...
    json2 = File.read('object_attributes.json')
    object_attributes = JSON.parse(json2)
    
    json3 = File.read('policy.json')
    policy = JSON.parse(json3)

    json4 = File.read('config.json')
